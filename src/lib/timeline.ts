...
/**
 * Resolves a list of evidence ids (a `PolicyGate.evidenceIds` or a story step's
 * `evidenceIds` — both point into the timeline the same way) to the actual events. Shared by
 * `lib/gates.ts` and `lib/story.ts` rather than duplicated, since it's the identical lookup
 * both need. An id that doesn't resolve to a real event is silently dropped — a
